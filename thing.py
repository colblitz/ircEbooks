...
class GuiThread(threading.Thread):
    def __init__(self, client, name=None):
        super(GuiThread, self).__init__(name=name)
        self.client = client
        self.options = ["asdf", "lgijwhoi4gh", "owhhuwiuh3"]
        self.buttons = []
        self.buttonValues = {}
    ...
    def updateValue(self):
        limit = int(self.filter.get())
        print "limit: ", limit

        for b in self.buttons:
            b.pack_forget()
            if len(self.buttonValues[b]) >= limit:
                b.pack(side=TOP)


    def doSearch(self):
        searchText = self.searchField.get()
        self.log("doSearch: {}".format(searchText))
        ...
        w = Spinbox(self.optionsPanel, from_=1, to=5, command=self.updateValue)
        w.pack(side=TOP)
        self.filter = w

        for a in range(random.randint(1, 20)):
            l = range(random.randint(1, 5))
            button = Button(self.optionsPanel, text=str(a)+str(l), command=lambda text = str(a)+str(l): self.buttonPress(text))
            button.pack(side=TOP)
            self.buttons.append(button)
            self.buttonValues[button] = l


    def run(self):
        try:
            self.log("Starting GUI thread")
            root = Tk()

            row = Frame(root)
            ent = Entry(row)
            self.searchField = ent
            but = Button(row, text='Search', command=self.doSearch)

            options = Frame(root)
            self.optionsPanel = options
            ...
            root.bind('<Return>', (lambda event: self.doSearch()))
